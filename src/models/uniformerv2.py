...
class Transformer(BaseModule):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.view(L, N, T_down, C) # 197, 15, 8, 768
        x = x[0] # (15, 8, 768)
        residual = x.mean(1) # (15, 768)
...
class UniFormerV2(BaseModule):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x.size(): (8, 3, 15, 224, 224)
        x = self.conv1(x)  # (8, 768, 15, 14, 14)
        N, C, T, H, W = x.shape
        x = x.permute(0, 2, 3, 4, 1).reshape(N * T, H * W, C) # 120, 196, 768

        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)  # (120, 197, 768)
        x = x + self.positional_embedding.to(x.dtype) # (120, 197, 768)
        x = self.ln_pre(x) # (120, 197, 768)

        x = x.permute(1, 0, 2) # (197, 120, 768)
...
class UniFormerV2Classifier(nn.Module):
    def __init__(self, num_classes=1, pretrained='ViT-B/16', pool='avg', t_size=15):
        super().__init__()

        self.final = nn.Linear(768, out_features=num_classes)
        self.avg_pool = nn.AdaptiveAvgPool3d((1, 1, 1)) if pool =="avg" else nn.AdaptiveMaxPool3d((1, 1, 1))
        self.dropout = nn.Dropout(0.5)
        _initialize_weights(self)

        self.backbone = UniFormerV2(t_size=t_size, pretrained=pretrained)
        self.backbone.init_weights()

    def forward(self, x):
        x = torch.transpose(x, 1, 2) # 8, 3, 15, 224, 224
        x = self.backbone(x) # 15, 768
        # x = self.avg_pool(x)
        # x = self.dropout(x)
        # x = x.flatten(1)
        x = self.final(x)
